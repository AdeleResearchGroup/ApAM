...
public class CompositeFactory extends ComponentFactory {
    ...
    public CompositeFactory(BundleContext context, Element metadata) throws ConfigurationException {
         ...
         */
        apamComponent = metadata.getAttribute(CompositeFactory.COMPONENT_IMPLEMENTATION_PROPERTY,
                CompositeFactory.APAM_NAMESPACE);
        apamSpecification = metadata.getAttribute(CompositeFactory.COMPONENT_SPECIFICATION_PROPERTY,
                CompositeFactory.APAM_NAMESPACE);

        if (apamComponent == null)
            apamComponent = "";
        if (apamSpecification == null)
            apamSpecification = "";
    ...
    public void check(Element metadata) throws ConfigurationException {
        ...
        String implementation = metadata.getAttribute(CompositeFactory.COMPONENT_IMPLEMENTATION_PROPERTY,
                CompositeFactory.APAM_NAMESPACE);
        String specification = metadata.getAttribute(CompositeFactory.COMPONENT_SPECIFICATION_PROPERTY,
                CompositeFactory.APAM_NAMESPACE);

        if ((implementation == null) && (specification == null)) {
            throw new ConfigurationException(
                    "A composite needs to specify the main implementation (by an implementation or specification name) : "
                            + metadata);
        }
        ...
        if (encodedServiceInterfaces == null) {
            throw new ConfigurationException(
                    "A composite needs to specify the provided interfaces in its provide clause : " + metadata);
        }
