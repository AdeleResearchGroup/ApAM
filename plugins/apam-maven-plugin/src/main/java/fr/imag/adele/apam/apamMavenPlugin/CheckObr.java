...
public class CheckObr {
	...
	private static void checkConstraint(ComponentDeclaration component,
		...
		checkFilters(component, dep.getImplementationConstraints(),
				dep.getImplementationPreferences(), validAttrs, dep.getTarget()
				.getName());
	...
	private static boolean checkFunctionSubst(ComponentDeclaration component,
		...
		/*
		 * Add a function that checks if the public method
		 * "public <type> func (Instance inst)" is realy defined in the class,
		 * and if its returned type is compatible with the "type"
		 */
	...
	private static boolean isSubstitute(ComponentDeclaration component,
			String attr) {
	...
	private static boolean checkFilter(ApamFilter filt,
		...
		case ApamFilter.OR: {
			...
			for (ApamFilter filter : filters) {
				if (!checkFilter(filter, component, validAttr, f, spec))
					ok = false;
			}
			return ok;
		}
		...
		case ApamFilter.PRESENT: {
			...
			if (validAttr.containsKey(filt.attr)) {
				if (isSubstitute(component, filt.attr)) {
					error("Filter attribute  " + filt.attr
							+ " is a substitution: .  Invalid constraint " + f);
					return false;
				}
				// if (Attribute.checkAttrType(name, defaultValue, type) !=
				// null) {
				// return checkSubstitute (component, name, type, defaultValue)
				// ;
				...
				// return Attribute.checkAttrType(attr, (String)value,
				// validAttr.get(attr)) != null;
			}
		}
		return true;
		}
	...
	private static boolean checkSubstitute(ComponentDeclaration component,
			String attr, String type, String defaultValue) {
		...
		if (source == null)
			return false;
		if (source == ApamCapability.trueCap)
			return true;
	...
	/**
	 * expecting that the relation will be inherited, create a dummy capability and dummy component that only refers to its group
	 * in order to get relation definitions to continue navigating
	 * @param source
	 * @return
	 */
	private static ApamCapability buildDummyImplem (ApamCapability source) {
		if (source.dcl instanceof SpecificationDeclaration) {
			AtomicImplementationDeclaration bidon = new AtomicImplementationDeclaration(
					"void-" + source.getName(), ((SpecificationReference) source.dcl.getReference()), null);
	...
	private static ApamCapability buildDummyInst (ApamCapability source) {
		if (source.dcl instanceof ImplementationDeclaration) {
			InstanceDeclaration bidon = new InstanceDeclaration(
					((ImplementationReference<?>) source.dcl.getReference()), source.getName() + "-01", null);
			return new ApamCapability(bidon);
		}
		return null;
	}
	...
	/**
	 * Compute the set of objects destination of a navigation from source through  final relation depname.
	 * If the navigation cannot be checked, return the capability "ApamCapability.trueCap",
	 * if it is invalid, return "null", otherwise returns the target capabilities.
	 * @param source
	 * @param depName
	 * @return
	 */
...
//		}
		...
		if (depName.equals(CST.REL_SPEC)) {
			...
			if (source.dcl instanceof ImplementationDeclaration) 
				return source.getGroup() ;
			if (source.dcl instanceof InstanceDeclaration) 
	...
	public static RelationDeclaration computeGroupRelation(
		 ...
		 */
		ComponentDeclaration group = ApamCapability.getDcl(depComponent
				.getGroupReference());
		while (group != null) {
			...
			if (relationTargetDefined && groupTargetdefined) {
				...
				if (relationTargetResource != null
					...
					ComponentDeclaration groupTarget = ApamCapability
	...
	private static boolean checkFilters(ComponentDeclaration component,
			Set<String> filters, List<String> listFilters,
			Map<String, String> validAttr, String comp) {
		boolean ok = true;
		if (filters != null) {
			for (String f : filters) {
				ApamFilter parsedFilter = ApamFilter.newInstance(f);
				if (parsedFilter == null
						|| !checkFilter(parsedFilter, component, validAttr, f,
								comp)) {
					ok = false;
				}
			}
	...
	private static void checkOwn(CompositeDeclaration component) {
		...
		for (OwnedComponentDeclaration own : owned) {
			...
			ComponentReference<?> foundReference = ApamCapability.getDcl(
