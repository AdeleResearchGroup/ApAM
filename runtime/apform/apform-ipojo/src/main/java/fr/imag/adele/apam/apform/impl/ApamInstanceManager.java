...
public class ApamInstanceManager extends InstanceManager implements
	...
	if (isApamCreated || (declaration == null)) {
	    declaration = new InstanceDeclaration(getFactory().getApform()
		    .getDeclaration().getReference(), instanceName, null);
	    for (Enumeration<String> properties = configuration.keys(); properties
		    .hasMoreElements();) {
	...
	RelationDefinition relDef = getApform().getApamComponent().getRelation(
		injection.getRelationInjection().getRelation().getIdentifier());
	...
	public void setApamComponent(Component apamComponent) {
		...
		fireCallbacks(AtomicImplementationDeclaration.Event.INIT,
			this.apamComponent);
		return;
	    }
		...
		fireCallbacks(AtomicImplementationDeclaration.Event.REMOVE,
			previousComponent);
	...
	public boolean setLink(Component destination, String depName) {
	     ...
	     */
	    for (RelationInjectionManager injectedField : injectedFields) {
		if (injectedField.getRelationInjection().getRelation()
			.getIdentifier().equals(depName)) {
