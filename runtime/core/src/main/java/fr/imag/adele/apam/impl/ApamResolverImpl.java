...
public class ApamResolverImpl implements ApamResolver {
	...
	if (source instanceof Instance) {
	    ...
	    compoType = ((Implementation) source).getInCompositeType()
		    .iterator().next();
	} else {
	    logger.error("Should not call deployedImpl on a source Specification "
		    + source);
	...
	while (!this.enabled) {
		...
		if (currentTime > maxDisableTime) {
		    ...
		    System.err
			    .println("APAM RESOLVER resuming resolution, condition did not happen: "
				    + condition);
		    enable();
		    return;
		}
    ...
    private Component findByName(Component client,
	    ComponentReference<?> targetComponent, ComponentKind targetKind) {
	if (client == null) {
	...
	RelationDefinition rel = new RelationDefinitionImpl(targetComponent,
		client.getKind(), targetKind, null, null);
	...
	return (Implementation) findByName(
		client,
		new ImplementationReference<ImplementationDeclaration>(implName),
		ComponentKind.IMPLEMENTATION);
    }

    @Override
    public Instance findInstByName(Component client, String instName) {
	return (Instance) findByName(client, new InstanceReference(instName),
	...
	return (Specification) findByName(client, new SpecificationReference(
		specName), ComponentKind.SPECIFICATION);
    }
	...
	logger.debug("creating a dummy root composite type " + applicationName
		+ " to contain unused " + mainInst);
     ...
     */
    private RelationDefinition getPromotionRel(Instance client,
	...
	List<RelationManager> externalPath = new ArrayList<RelationManager>();
	for (RelationManager relationManager : ApamManagers
		.getRelationManagers()) {
	    relationManager
		    .getSelectionPath(source, relToResolve, externalPath);
	}
	...
	if (!relToResolve.isRelation()) { // It is a find
	    logger.info("Looking for " + relToResolve.getTargetKind() + " "
		    + relToResolve.getTarget().getName());
	} else {
	...
	for (RelationManager manager : selectionPath) {
	    if (manager == null) {
		throw new RuntimeException("Manager is null, SelectionPath "
			+ selectionPath);
	    }
	    if (manager.getName() == null) {
		throw new RuntimeException("Manager : " + manager
	     ...
	     */
	    Component depl = (res.toInstantiate != null) ? res.toInstantiate
		    : res.singletonResolved;
		...
		if (relToResolve.getTargetKind() != ComponentKind.INSTANCE) {
		    logger.error(mess
			    + "Invalid Resolved value. toInstantiate is set, but target kind is not Instance");
		    continue;
		}
		...
		if (resolveExist) {
		    logger.error(mess
			    + "create=\"exist\" but only an implementations was found : "
			    + res.toInstantiate
			    + " cannot instantiate. Resolve failed.");
		    continue;
		}

		Composite compo = (source instanceof Instance) ? ((Instance) source)
			.getComposite() : CompositeImpl.getRootInstance();
		Instance inst = res.toInstantiate.createInstance(compo, null);
		if (inst == null) { // may happen if impl is non instantiable
		    logger.error(mess + "Failed creating instance of "
			    + res.toInstantiate);
		    continue;
		}

		if (!relToResolve.matchRelationConstraints(inst)) {
		    logger.debug(mess + " Instantiated instance " + inst
		...
		if (res.setResolved == null || res.setResolved.isEmpty()) {
		    logger.info(mess + "manager " + manager
			    + " returned an empty result. Should be null.");
		    continue;
		}
		if (((Component) res.setResolved.iterator().next()).getKind() != relToResolve
			.getTargetKind()) {
		    logger.error(mess + "Manager " + manager
	...
	RelationDefinition dep = new RelationDefinitionImpl(
		new ImplementationReference(impl.getName()), client.getKind(),
		ComponentKind.INSTANCE, constraints, preferences);
	...
	if (source == null) {
	    logger.error("Component source not at the right level; found "
		    + source2 + " expected " + rel.getSourceKind());
	    return null;
	}
	 ...
	 */
	if (resolved == null || resolved.isEmpty()) {
	    resolved = apam.getFailedResolutionManager().resolveRelation(
		    source, relToResolve);
	}
    ...
    @Override
    public Implementation resolveSpecByInterface(Component client,
	    String interfaceName, Set<String> constraints,
	    List<String> preferences) {

	RelationDefinition dep = new RelationDefinitionImpl(
		new InterfaceReference(interfaceName), client.getKind(),
		ComponentKind.IMPLEMENTATION, constraints, preferences);
	return resolveSpecByResource(client, dep);
    }

    @Override
    public Implementation resolveSpecByMessage(Component client,
	    String messageName, Set<String> constraints,
	    List<String> preferences) {

	RelationDefinition dep = new RelationDefinitionImpl(
		new MessageReference(messageName), client.getKind(),
		ComponentKind.IMPLEMENTATION, constraints, preferences);
	return resolveSpecByResource(client, dep);
    }
	...
	RelationDefinition dep = new RelationDefinitionImpl(
		new SpecificationReference(specName), client.getKind(),
		ComponentKind.IMPLEMENTATION, constraints, preferences);
     ...
     * @return
     */
    public Implementation resolveSpecByResource(Component client,
	    RelationDefinition relDef) {
	if (relDef.getTargetKind() != ComponentKind.IMPLEMENTATION) {
	    logger.error("Invalid target type for resolveSpecByResource. Implemntation expected, found : "
		    + relDef.getTargetKind());
	    return null;
	}
