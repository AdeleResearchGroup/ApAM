...
public class ApamMan implements RelationManager {
	...
	if (relToResolve.getTarget() instanceof SpecificationReference) {
	    ...
	    System.err.println("Invalid target reference : "
		    + relToResolve.getTarget());
	    return null;
	}
	 ...
	 */
		...
		for (Specification spec : CST.componentBroker.getSpecs()) {
		    if (spec.getDeclaration().getProvidedResources()
			    .contains((relToResolve.getTarget()))) {
			specs.add(spec);
		    }
		}
		...
		if (impl.getDeclaration().getProvidedResources()
			.contains((relToResolve.getTarget()))) {
		    impls.add(impl);
		}
	    }
	...
	boolean fast = (!relToResolve.isMultiple() && !relToResolve
		.hasPreferences());

	Set<Instance> insts = new HashSet<Instance>();
	for (Implementation impl : impls) {
	    for (Instance inst : impl.getInsts()) {
		if (inst.isSharable() && source.canSee(inst)
	...
	for (Implementation impl : impls) {
	    if (relToResolve.matchRelationConstraints(
		    ComponentKind.IMPLEMENTATION, impl.getAllProperties())) {
		valid.add(impl);
	    }
	}
	if (valid.isEmpty()) {
