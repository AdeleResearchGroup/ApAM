...
public class UpdateMan implements RelationManager, DynamicManager {
	...
	try {
	    ...
	    List<RelationManager> selectionPath = ApamManagers
		    .getRelationManagers();
	    logger.info("Updating implementation " + implName
		    + " in composite " + compoTypeFrom);
		...
		sel = manager.findBundle(compoTypeFrom,
			bundle.getSymbolicName(), implName);
		...
		if (sel != null && sel.getComponents().contains(implName)) { // it
									     ...
									     // a
									     // deployment
		    logger.info("Updating component " + implName
			    + " in composite " + compoTypeFrom
	...
	Specification spec = CST.componentBroker.getSpecResource(dep
		.getTarget());
	if (spec == null) {
	    return null;
	}
