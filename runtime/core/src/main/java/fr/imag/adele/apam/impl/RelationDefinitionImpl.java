...
public class RelationDefinitionImpl implements RelationDefinition {
	...
	this.sourceKind = (declaration.getSourceKind() == null) ? ComponentKind.INSTANCE
		: declaration.getSourceKind();
	this.targetKind = (declaration.getTargetKind() == null) ? ComponentKind.INSTANCE
		: declaration.getTargetKind();
	...
	for (RequirerInstrumentation injection : declaration
		.getInstrumentations()) {
		...
		if (((RequirerInstrumentation.RequiredServiceField) injection)
			.isWire()) {
		    this.isWire = true;
		}
	    }
	...
	if (declaration.getCreationPolicy() == null) {
	    this.create = hasCallbacks ? CreationPolicy.EAGER
		    : CreationPolicy.LAZY;
	} else {
	    this.create = declaration.getCreationPolicy();
	}

	this.resolve = declaration.getResolvePolicy() == null ? ResolvePolicy.EXTERNAL
		: declaration.getResolvePolicy();
	this.missingPolicy = declaration.getMissingPolicy();
	this.missingException = declaration.getMissingException();

	this.mustHide = (declaration.isHide() == null) ? false : declaration
		.isHide();
	this.isDynamic = declaration.isMultiple()
		|| (this.create == CreationPolicy.EAGER);

	// Constraints
	this.implementationConstraints.addAll(declaration
		.getImplementationConstraints());
	this.instanceConstraints.addAll(declaration.getInstanceConstraints());
	this.implementationPreferences.addAll(declaration
    ...
    public RelationDefinitionImpl(ResolvableReference target,
	    ComponentKind sourceKind, ComponentKind targetKind,
	...
	this.sourceKind = (sourceKind == null) ? ComponentKind.INSTANCE
		: sourceKind;
	this.targetKind = (targetKind == null) ? ComponentKind.INSTANCE
		: targetKind;
	...
	return this.declaration != null ? this.declaration
		.refinedBy(refinement) : refinement;
    }
	...
	ret.append(" (creation = " + create + ", resolve = " + resolve
		+ ", missing policy = " + this.missingPolicy + ")");
