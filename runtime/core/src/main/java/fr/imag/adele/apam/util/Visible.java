...
public class Visible {
	...
	if (source instanceof Implementation) {
		...
		return isVisible((Implementation) source,
...
//TODO
    ...
    private static boolean isVisible(Implementation source,
	    Implementation target) {

	// They have a composite type in common
	Set<CompositeType> intersection = new HashSet<CompositeType>(
		source.getInCompositeType());
	...
	if (source.getInCompositeType().contains(
		target.getComposite().getCompType())) {
	    return true;
	}
	...
	// application
	String applicationExports = target.getComposite().getCompType()
		.getCompoDeclaration().getVisibility()
		.getApplicationInstances();
	if (matchVisibilityExpression(applicationExports, target, target
		.getComposite().getCompType())) {
	    return target.getAppliComposite()
		    .equals(source.getAppliComposite());
	}
     ...
     * type.
     */
    private static boolean isVisibleIn(CompositeType source,
	    Implementation target) {

	assert source != null && target != null
		&& !target.getInCompositeType().isEmpty();
	...
	String imports = source.getCompoDeclaration().getVisibility()
		.getImportImplementations();
	...
	for (CompositeType deployingTarget : target.getInCompositeType()) {
	    String exports = deployingTarget.getCompoDeclaration()
		    .getVisibility().getExportImplementations();
	...
	String exports = target.getComposite().getCompType()
		.getCompoDeclaration().getVisibility().getExportInstances();
     ...
     * 
     */
    private static boolean matchVisibilityExpression(String expre,
