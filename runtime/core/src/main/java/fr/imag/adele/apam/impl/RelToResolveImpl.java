...
public class RelToResolveImpl implements RelToResolve {
    ...
    public RelToResolveImpl(Component source, ResolvableReference target,
	...
	relationDefinition = new RelationDefinitionImpl(target, sourceKind,
		targetKind, constraints, preferences);
	linkSource = source;
    }
	...
	if (!getImplementationConstraints().isEmpty()) {
	    if (!((RelationDefinitionImpl) relationDefinition)
		    .isStaticImplemConstraints()) {
		for (String c : relationDefinition
		...
		allImplementationConstraintFilters
			.addAll(((RelationDefinitionImpl) relationDefinition)
				.getImplementationConstraintFilters());
	    }
	}
	...
	if (!getInstanceConstraints().isEmpty()) {
		...
		allInstanceConstraintFilters
			.addAll(((RelationDefinitionImpl) relationDefinition)
				.getInstanceConstraintFilters());
	    }
	}

	if (!getImplementationPreferences().isEmpty()) {
	    if (!((RelationDefinitionImpl) relationDefinition)
		    .isStaticImplemPreferences()) {
		for (String c : relationDefinition
		...
		allImplementationPreferenceFilters
			.addAll(((RelationDefinitionImpl) relationDefinition)
				.getImplementationpreferencfeFilters());
	    }
	}

	if (!getInstancePreferences().isEmpty()) {
	    if (!((RelationDefinitionImpl) relationDefinition)
		    .isStaticInstPreferences()) {
		...
		allInstancePreferenceFilters
			.addAll(((RelationDefinitionImpl) relationDefinition)
				.getInstancePreferenceFilters());
	    }
	}
	...
	return getDeclaration() != null ? getDeclaration()
	...
	ret.append(" (creation = " + relationDefinition.getCreation()
		+ ", resolve = " + relationDefinition.getResolve()
		+ ", missing policy = " + relationDefinition.getMissingPolicy()
		+ ")");
