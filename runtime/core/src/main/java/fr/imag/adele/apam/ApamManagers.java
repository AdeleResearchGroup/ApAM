...
public class ApamManagers {
     ...
     */
    private static Set<Manager> managers = new ConcurrentSkipListSet<Manager>(
    ...
    private static Set<DynamicManager> dynamicManagers = new ConcurrentSkipListSet<DynamicManager>(
	    new Comparator<DynamicManager>() {

		@Override
		public int compare(DynamicManager manager1,
			DynamicManager manager2) {
		    return manager1.getName().compareTo(manager2.getName());
		}
	    });
    ...
    private static Set<PropertyManager> propertyManagers = new ConcurrentSkipListSet<PropertyManager>(
	    new Comparator<PropertyManager>() {

		@Override
		public int compare(PropertyManager manager1,
	...
	if ((priority < 0)
		&& !(manager.getName().equals(CST.APAMMAN) || manager.getName()
			.equals(CST.UPDATEMAN))) {
     ...
     */
    public static void notifyAttributeAdded(Component component, String attr,
	    String value) {
    ...
    public static void notifyAttributeChanged(Component component, String attr,
    ...
    public static void notifyAttributeRemoved(Component component, String attr,
	    String oldValue) {
	for (PropertyManager manager : ApamManagers.propertyManagers) {
	...
	if (manager.getName() != null) {
	    ...
	    ManagerModel rootModel = CompositeTypeImpl.getRootCompositeType()
		    .getModel(manager.getName());
	    // the root model maybe null
	    manager.newComposite(rootModel,
	...
	unregister(manager);
	if (manager != null) {
	    boolean managerRemoved = ApamManagers.dynamicManagers
		    .remove(manager);
	    if (!managerRemoved) {
		logger.error("impossible to remove dynamic manager {}",
			manager.getName());
	    }
	}
    }
	...
	unregister(manager);
	if (manager != null) {
	    boolean managerRemoved = ApamManagers.propertyManagers
		    .remove(manager);
	    if (!managerRemoved) {
		logger.error("impossible to remove property manager {}",
			manager.getName());
	    }
	}
    }
	...
	if (!removedManagerPrio) {
	    logger.error("impossible to remove manager {} from prior list",
		    manager.getName());
	}
	if (!removedManager) {
	    logger.error("impossible to remove manager {} from main list",
		    manager.getName());
	}
