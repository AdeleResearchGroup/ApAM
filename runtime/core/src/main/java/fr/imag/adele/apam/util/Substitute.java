...
public class Substitute {
    ...
    private static Object checkReturnSub(Component source, SplitSub sub,
	    String sourceAttr, AttrType sourceTypeAttr) {
     ...
     * @return
     */
    public static boolean checkSubType(AttrType sourceType,
	    AttrType targetType, String attr, SplitSub sub) {
	...
	if (sub.prefix != null || sub.suffix != null) { // The result is a
	    ...
	    logger.error("Attribute " + attr + " of type "
		    + sourceType.typeString
		    + " : invalid substitution with string \"" + sub
		    + "\". Attribute " + sub.attr + " of type : "
	...
	if (sourceType.type != targetType.type) {
	    logger.error("Attribute " + attr + " of type "
		    + sourceType.typeString + " : invalid substitution with \""
		    + sub + "\". Attribute " + sub.attr + " of type : "
		    + targetType.typeString);
	    return false;
	}

	if (sourceType.type == AttrType.ENUM
		&& !sourceType.enumValues.containsAll(targetType.enumValues)) {
	    logger.error("Attribute " + attr + " of type "
		    + sourceType.typeString
		    + " : Not the same enumeration set as attribute "
    ...
    public static Object functionSubstitute(String attr, String value,
	    Component source) {
	if (!(source instanceof Instance)) {
	    logger.error("Invalid function substitution. " + source.getName()
		    + " is not an instance.");
	}
	...
	try {
		...
		if (m.getName().equals(func)) {
		    Object ret = m.invoke(
			    ((InstanceImpl) source).getServiceObject(),
			    (Instance) source);
		    return ret;
		}
	    }
	    logger.error("Not found method " + func + " in class "
		    + c.getCanonicalName() + " of instance " + source);
	    return null;
	} catch (Exception e) {
	    logger.error("Invalid invoke on " + func + " in class "
		    + c.getCanonicalName() + " of instance " + source);
	    e.printStackTrace();
	}
	return null;
	...
	return ((value instanceof String) && !((String) value).isEmpty() && (((String) value)
		.charAt(0) == '$' || ((String) value).charAt(0) == '@'));
    }
     ...
     * @return
     */
    private static Set<Component> navigate(Component firstSource,
	...
	for (String depId : navigation) {
		...
		if (depDcl == null && !CST.isFinalRelation(depId)) {
			...
			logger.debug("relation id " + depId
				+ " not resolved for component "
				+ source.getName());
		    } else {
	...
	return new SplitSub(attr, prefix.substring(0, j),
     ...
     */
    @SuppressWarnings("unchecked")
    public static Object substitute(String attr, Object valueObject,
	    Component source) {

	/*
	 * No substitution cases
	 */
	if (source == null || (valueObject == null)
	...
	if (!sub.sourceName.equals("this")) {
	    ...
	    Component newSource = CST.componentBroker
		    .getComponent(sub.sourceName);
	    if (newSource == null) {
		logger.error("Component " + sub.sourceName
			+ " not found in substitution : " + value
			+ " of attribute " + attr);
		return null;
	    }
		...
		logger.error("Component " + sub.sourceName
			...
			+ " in substitution : " + value + " of attribute "
			+ attr);
		return null;
	    }
	    source = newSource;
	...
	if (!st.isSet) {
	    logger.error("Invalid type for attribute " + attr
		    + " It must be a set for a multiple relation or navigation"
		    + sub.depIds);
	    return null;
	}
     ...
     * @return
     */
    public static List<ApamFilter> toFiltersSubst(List<String> filterString,
	    Component component) {
	if (component == null) {
	    return null;
	}
     ...
     * @return
     */
    public static Set<ApamFilter> toFiltersSubst(Set<String> filterString,
	    Component component) {
	if (component == null) {
	    return null;
	}
