...
public class FailedResolutionManager implements RelationManager,
	DynamicManager, PropertyManager {
     ...
     */
    private static void throwMissingException(Component source,
	...
	try {
		...
		RelationDeclaration declaration = declaringComponent
			.getDeclaration().getLocalRelation(
				relToResolve.getName());
		if (declaration != null
			&& declaration.getMissingException() != null
			&& declaration.getMissingException().equals(
				exceptionName)) {
		    break;
		}
		...
		declaringComponent = ((Instance) source).getComposite()
			.getCompType();
	    }
	    ...
	    Class<?> exceptionClass = declaringComponent.getApformComponent()
		    .getBundle().loadClass(exceptionName);
	    Exception exception = Exception.class.cast(exceptionClass
		    .newInstance());
    ...
    @Override
    public void getSelectionPath(Component client, RelToResolve relToResolve,
     ...
     */
    @Override
    public Resolved<?> resolveRelation(Component client,
	    RelToResolve relToResolve) {
	...
	case WAIT: {
	     ...
	     * schedule request
	     */
	    PendingRequest request = new PendingRequest(CST.apamResolver,
		    client, relToResolve.getRelationDefinition());
