...
public abstract class ComponentImpl extends ConcurrentHashMap<String, Object> implements Component, Comparable<Component> {
	...
	public <T extends Component> Set<T> getConstraintsPreferedComponent(Set<T> candidates, List<Filter> preferences, Set<Filter> constraints) {
		...
		Set<T> valids = getSelectedComponents(candidates, constraints);
	...
	public <T extends Component> T getPreferedComponentFromMembers(Set<T> candidates, List<Filter> memberPreferences, Set<Filter> memberConstraints) {
        ...
        Set<Component> allMembers = new HashSet<Component> ();
        for (T compo : candidates) {
        	allMembers.addAll(compo.getMembers());
        }
        if (allMembers.isEmpty()) return candidates.iterator().next () ;
        
        Set<Component> valids = getConstraintsPreferedComponent(allMembers, memberPreferences, memberConstraints) ;
        if (valids.isEmpty()) return candidates.iterator().next () ;

        return (T)valids.iterator().next().getGroup() ;
	}
//        for (T compo : candidates) {
...
//			
//			if (!getConstraintsPreferedComponent(members, memberPreferences, memberConstraints).isEmpty())
//				valids.add (compo) ;
//        }
//		
//		if (valids.isEmpty()) return getDefaultComponent(candidates) ;
//		return getDefaultComponent(valids) ;	
//    }
