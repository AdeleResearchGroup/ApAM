...
public class Attribute {
    ...
    public static Object checkAttrType(String attribute, Object value,
	    String typeString) {
	if ((typeString == null) || (value == null) || typeString.isEmpty()
		|| attribute == null || attribute.isEmpty()) {
	    logger.error("Invalid property " + attribute + " = " + value
		    + " type=" + typeString);
	    return null;
	}
	...
	if (aType.type == AttrType.INTEGER) { // aType.singletonType.equals
		...
		if (!(value instanceof Set<?>)) {
		    logger.error("Attribute value " + value
			    + " not an a Set<Integer> for attribute "
			    + attribute);
		    return false;
		}
		...
		try {
		    ...
		    logger.error("In attribute value " + value
			    + " is not an Integer Set, for attribute "
			    + attribute);
		    return false;
		}
	    ...
	    logger.error("Invalid integer value " + value + " for attribute "
		    + attribute);
	    return false;
	}
	...
	if (aType.type == AttrType.BOOLEAN) {
	    ...
	    logger.error("Invalid value: not a Boolean " + value
	 ...
	 */
	if (!aType.isSet) {
	    logger.error("Invalid value: not a single String " + value
		    + " for attribute " + attribute);
	    return null;
	}
	if (!(value instanceof Set)) {
	    logger.error("Invalid value: not a Set of String " + value
		    + " for attribute " + attribute);
	    return null;
	}
	...
	for (Object i : (Set<?>) value) {
	    if (!(i instanceof String)) {
		logger.error("In attribute value " + value + ", "
			+ i.toString() + " is not a String, for attribute "
			+ attribute);
		return false;
	    }
	}
	...
	if (aType.enumValues.containsAll((Set<String>) value)) {
	    ...
	    logger.error("Invalid value " + value + " for attribut "
		    + attribute + ". Expected subset of " + aType.typeString);
	    return false;
	}
    };
     ...
     *            "{string}" or enumeration "{v1, v2, v3 ...}"
     */
    private static Object checkAttrTypeString(String attr, String value,
	    AttrType at) {

	Set<String> values = Collections.unmodifiableSet(Util.splitSet(value));
	if (values.size() > 1 && !at.isSet) {
	    logger.error("Values are a set \"" + values + "\" for attribute \""
		    + attr + "\". while type is singleton: \"" + at.typeString
		    + "\"");
	    return null;
	}
	...
	case AttrType.ENUM:
		...
		logger.error("Invalid attribute value(s) \"" + value
			+ "\" for attribute \"" + attr
			+ "\".  Expected subset of: "
			+ Util.stringSet2String(at.enumValues));
		return null;
	    }
	    if (at.enumValues.contains(value)) {
		return value;
	    }
	    logger.error("Invalid attribute value \"" + value
		    + "\" for attribute \"" + attr
	...
	case AttrType.INTEGER:
		...
		for (String val : values) {
		    normalizedValues
			    .add(Integer.toString(Integer.parseInt(val)));
		}
	...
	if (isFinalAttribute(attr)) {
	    logger.error("ERROR: in " + component + ", attribute\"" + attr
		    + "\" is final");
	    return false;
	}
	...
	if (type == null
		|| !(type.equals("string") || type.equals("int")
			|| type.equals("boolean") || type.charAt(0) == '{')) {
	    logger.error("Invalid type "
		    + type
		    + ". Supported: string, int, boolean, enumeration; and sets");
	    return false;
	}
	return true;
    }
