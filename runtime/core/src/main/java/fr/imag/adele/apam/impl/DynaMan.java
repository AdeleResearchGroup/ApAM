...
public class DynaMan implements DynamicManager, PropertyManager {
	...
	for (RelationDefinition relDef : component.getRelations()) {
	    ...
	    if (component.getKind().equals(relDef.getSourceKind())
		    && relDef.isDynamic()) {
	...
	for (InstanceDeclaration instanceDeclaration : composite.getCompType()
		.getCompoDeclaration().getInstanceDeclarations()) {
	    try {
		 ...
		 */
		FutureInstance request = new FutureInstance(composite,
			instanceDeclaration);
		...
		logger.error("Error managing dynmaic instances for composite "
			+ composite.getName(), error);
	    }
	}
    }
