...
/**
 * This class implements the manager in charge of dynamically updating the architecture, in response
 * to execution context evolution.
 * 
 * @author vega
 *
 */
...
public class DynaMan implements DynamicManager, PropertyManager {
	...
	public void addedComponent(Component component) {
		...
		/*
		 * Add the dynamic dependencies and future instances of the
		...
		/*
		 * Verify if the new component satisfies some pending requests or 
	...
	public void removedComponent(Component component) {
		...
		/*
		 * Remove from the list of dynamic requests all requests originating from the
		 * removed component
		 */
		...
		/*
		 * Remove from the list of future instances all requests originating from the
		 * removed component
		 */
	...
	public void ownershipChanged(Instance instance) {
		/*
		 * Verify if the component in the new context satisfies some pending requests or 
		 * the triggering condition of future instances
		 */
	...
	private void addFutureInstances(Composite composite) {
		for (InstanceDeclaration instanceDeclaration : composite.getCompType().getCompoDeclaration().getInstanceDeclarations()) {
			try {
				...
				/*
				 * Create the future instance request and evaluate triggering conditions
	...
	/**
	 * Verifies if the triggering conditions of pending future instances are satisfied
	 */
	private void resolveFutureInstances(Component component) {
		
		/*
		 * Future instances are triggered when a matching instance is added to the composite
		 * where the future is declared
		 */
		...
		synchronized (futureInstances) {
			...
			while(! futureInstances.isEmpty()) {
				 ...
				 * so that we are sure that there is only a single invocation (in case of
				 * nested invocations) of this method handling a given pending instance 
				 */
