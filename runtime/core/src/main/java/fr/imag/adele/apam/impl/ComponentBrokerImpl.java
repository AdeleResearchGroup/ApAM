...
public class ComponentBrokerImpl implements ComponentBroker {
     ...
     * structure.
     */
    private static class ApamOnlySpecification extends
	    BaseApformComponent<Specification, SpecificationDeclaration>
	    implements ApformSpecification {

	public ApamOnlySpecification(String name,
    ...
    private static Logger logger = LoggerFactory
	    .getLogger(ComponentBrokerImpl.class);
    /*
     * The three main table to maintain.
     */
    private static final Set<Specification> specifications = Collections
	    .newSetFromMap(new ConcurrentHashMap<Specification, Boolean>());
    private static final Set<Implementation> implementations = Collections
	    .newSetFromMap(new ConcurrentHashMap<Implementation, Boolean>());

    private static final Set<Instance> instances = Collections
	...
	assert implementation != null
		&& !implementations.contains(implementation);
	...
	if (implementation != null) {
	    logger.error("Error adding implementation: already exists"
		    + implementationName);
	...
	try {
		...
		implementation = new CompositeTypeImpl(composite,
			(ApformCompositeType) apfImpl);
	    } else {
	    ...
	    logger.error(
		    "Error adding implementation: exception in APAM registration, with message: {}",
	...
	String implementationName = apfInst.getDeclaration()
		.getImplementation().getName();

	Instance instance = getInst(instanceName);
	if (instance != null) {
	    logger.error("Error adding instance: already exists "
		    + instanceName);
	    return instance;
	}
	...
	Implementation implementation = getImpl(implementationName);
	if (implementation == null) {
	    logger.error("Error adding instance: implementation not found"
		    + implementationName);
	    return null;
	}
	...
	try {
	    ...
	    instance = ((ImplementationImpl) implementation).reify(composite,
		    apfInst);
	    ...
	    logger.error(
		    "Error adding instance: exception in APAM registration",
		    configurationError);
	}
	...
	if (specification != null) {
	    logger.error("Error adding specification: already exists "
		    + specificationName);
	...
	try {
	    ...
	    logger.error(
		    "Error adding specification: exception in apam registration",
		    configurationError);
	}
	...
	return addSpec(new ApamOnlySpecification(specName, resources,
		properties));
    }
	...
	if (service instanceof Pojo) {
		...
		ApformInstance apform = (ApformInstance) pojo
