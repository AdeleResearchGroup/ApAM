...
public class ImplementationImpl extends ComponentImpl implements Implementation {
     ...
     * 
     */
    private static class SystemRootImplementation extends
	    BaseApformComponent<CompositeType, ImplementationDeclaration>
	    implements ApformImplementation {

	public SystemRootImplementation(String name) {
	    super(new CompositeDeclaration(name, (SpecificationReference) null,
	...
	public ApformInstance addDiscoveredInstance(
	    ...
	    throw new UnsupportedOperationException(
		    "method not available in root type");
	}
    ...
    private static Logger logger = LoggerFactory
	    .getLogger(ImplementationImpl.class);

    private static final long serialVersionUID = 1L;

    // composite in which it is contained
    private Set<CompositeType> inComposites = Collections
    ...
    // the instances
    private Set<Instance> instances = Collections
     ...
     */
    protected ImplementationImpl(CompositeType composite,
	    ApformImplementation apfImpl) throws InvalidConfiguration {
	super(apfImpl);
	 ...
	 */
	if (mySpec == null) {
	    mySpec = CST.componentBroker.createSpec(declaration.getName()
		    + "_spec", declaration.getProvidedResources(),
	...
	mySpec = CST.componentBroker.createSpec(name + "_spec",
		new HashSet<ResourceReference>(), null);
    }
     ...
     */
    @Override
    public Instance createInstance(Composite composite,
	    Map<String, String> initialProperties) {

	// if ((composite != null) &&
	// !Util.checkImplVisible(composite.getCompType(), this)) {
	if ((composite != null) && !this.canBeInstantiatedIn(composite)) {
	    logger.error("cannot instantiate " + this
		    + ". It is not visible from composite " + composite);
	    return null;
	}
	...
	try {
	    ...
	    logger.error("Error instantiating implementation " + this.getName()
		    + ": exception registering instance in APAM "
		    + configurationError.getMessage());
	}
	return null;
    }
    ...
    protected Instance instantiate(Composite composite,
	    Map<String, String> initialProperties) throws InvalidConfiguration {

	if (!this.isInstantiable()) {
	    throw new InvalidConfiguration("Implementation " + this
		    + " is not instantiable");
	}

	if (this.isSingleton() && !instances.isEmpty()) {
	    throw new InvalidConfiguration("Implementation " + this
		    + " is a singleton and an instance exists");
	}

	return reify(composite,
		getApformImpl().createInstance(initialProperties));
    }
     ...
     * 
     */
    protected Instance reify(Composite composite,
