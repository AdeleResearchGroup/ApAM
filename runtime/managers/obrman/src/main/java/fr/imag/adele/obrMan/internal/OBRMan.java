...
public class OBRMan implements RelationManager, OBRManCommand {
	...
	for (Bundle bundle : bundles) {
	    if (bundle.getSymbolicName() != null
		    && bundle.getSymbolicName().equals(bundleName)) {
	...
	Selected selected = obrManager.lookFor(CST.CAPABILITY_COMPONENT,
		"(name=" + rel.getTarget().getName() + ")", rel);
	fr.imag.adele.apam.Component c = installInstantiate(selected);
	if (c == null) {
	    return null;
	}
	if (c.getKind() != rel.getTargetKind()) {
	    logger.debug("ERROR : " + rel.getTarget().getName()
		    + " is found but is an " + rel.getTargetKind() + " not an "
		    + c.getKind());
	}
    ...
    // at the end
    @Override
    public void getSelectionPath(Component client, RelToResolve dep,
	    List<RelationManager> involved) {
	 ...
	 */
	fr.imag.adele.apam.Component c = CST.componentBroker
		.getComponent(selected.getComponentName());
	if (c != null) {
	    return c;
	}
	 ...
	 */
	if (theBundle.getState() == Bundle.ACTIVE
		|| theBundle.getState() == Bundle.STARTING) {
	    if (theBundle.getVersion().equals(selected.resource.getVersion())) {
		return waitAndReturnComponent(selected);
	    }
	    logger.error("Bundle " + selected.getComponentName()
		    + " is already installed under version "
		    + theBundle.getVersion()
		    + " while trying to deploy version "
		    + selected.resource.getVersion());
	    return null;
	}
	 ...
	 */
	if (theBundle.getState() == Bundle.INSTALLED
		|| theBundle.getState() == Bundle.RESOLVED) {
	    try {
		theBundle.start();
	    } catch (BundleException e) {
		// Starting failed. No solution : cannot be deployed and cannot
		// be started. Make as if failed
		logger.info("The bundle " + theBundle.getSymbolicName()
			+ " is installed but cannot be started!");
		return null;
	    }

	    logger.info("The bundle " + theBundle.getSymbolicName()
		    + " is installed and has been be started!");
	    return waitAndReturnComponent(selected);
	}
	return null;
    }
	...
	if (resource instanceof SpecificationReference) {
	    selected = obrManager.lookFor(
		    CST.CAPABILITY_COMPONENT,
		    "(provide-specification*>"
			    + resource.as(SpecificationReference.class)
				    .getName() + ")", dep);
	}
	if (resource instanceof InterfaceReference) {
	    selected = obrManager.lookFor(
		    CST.CAPABILITY_COMPONENT,
		    "(provide-interfaces*>"
			    + resource.as(InterfaceReference.class)
				    .getJavaType() + ")", dep);
	}
	if (resource instanceof MessageReference) {
	    selected = obrManager.lookFor(CST.CAPABILITY_COMPONENT,
		    "(provide-messages*>"
	...
	if ((dep.getTarget() instanceof ResourceReference)
		|| (((ComponentReference<?>) dep.getTarget()).getKind() == ComponentKind.SPECIFICATION && dep
	...
	if (badKind) {
	    logger.debug("Looking for " + dep.getTargetKind() + " but found "
		    + ret);
	    // It is Ok to return an implem when an instance is required. The
	    // resolver will instantiate.
	    if (!(ret.getKind() == ComponentKind.IMPLEMENTATION && dep
		    .getTargetKind() == ComponentKind.INSTANCE)) {
		logger.error("invalide return from OBR : expected "
			+ dep.getTargetKind() + " got " + ret.getKind());
		return null;
	    }
	}
	...
	if (obrManager == null) {
		...
		String rootModelurl = m_context
			.getProperty(ObrUtil.ROOT_MODEL_URL);
		try {// try to load root obr model from the customized location
			...
			obrManager = new OBRManager(this,
				CST.ROOT_COMPOSITE_TYPE, repoAdmin, obrModel);
		    ...
		    obrModel.put(ObrUtil.LOCAL_MAVEN_REPOSITORY, "true");
		    obrModel.put(ObrUtil.DEFAULT_OSGI_REPOSITORIES, "true");
		    obrManager = new OBRManager(this, CST.ROOT_COMPOSITE_TYPE,
			    repoAdmin, obrModel);
		    obrManagers.put(CST.ROOT_COMPOSITE_TYPE, obrManager);
		}
	    }
	...
	Component c = CST.componentBroker.getWaitComponent(
		selected.getComponentName(), timeout);
