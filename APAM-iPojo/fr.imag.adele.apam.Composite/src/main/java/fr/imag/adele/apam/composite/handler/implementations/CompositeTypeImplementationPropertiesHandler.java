...
public class CompositeTypeImplementationPropertiesHandler extends ImplementationHandler {
    ...
    public void initializeComponentFactory(ComponentTypeDescription componentDescriptor, Element componentMetadata) throws ConfigurationException {
    	...
    	if (!(componentDescriptor instanceof CompositeType.Description))
	        throw new ConfigurationException("APAM implementations handler can only be used on APAM composite types"+componentMetadata);
        ...
        Element propertiesDeclarations[] = componentMetadata.getElements(IMPLEMENTATIONS_DECLARATION,APAM_NAMESPACE);
         ...
         */
        String internal = propertiesDeclaration.getAttribute(PROPERTY_COMPOSITE_INTERNAL_IMPLEMENTATION);
        if (internal != null)
        	addProperty(compositeTypeDescription,CST.A_INTERNALIMPL,internal);
        
        String local = propertiesDeclaration.getAttribute(PROPERTY_COMPOSITE_LOCAL_VISIBILITY);
        if (local != null)
        	addProperty(compositeTypeDescription,CST.A_LOCALVISIBLE,local,"String[]");
 
        String composite = propertiesDeclaration.getAttribute(PROPERTY_COMPOSITE_COMPOSITE_VISIBILITY);
        if (composite != null)
        	addProperty(compositeTypeDescription,CST.A_COMPOSITEVISIBLE,"String[]");
