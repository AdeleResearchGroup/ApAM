...
public class CompositeTypeInstancePropertiesHandler extends ImplementationHandler {
    ...
    public void initializeComponentFactory(ComponentTypeDescription componentDescriptor, Element componentMetadata) throws ConfigurationException {
    	...
    	if (!(componentDescriptor instanceof CompositeType.Description))
	        throw new ConfigurationException("APAM instances handler can only be used on APAM composite types"+componentMetadata);
        ...
        Element propertiesDeclarations[] = componentMetadata.getElements(INSTANCES_DECLARATION,APAM_NAMESPACE);
         ...
         */
        String internal = propertiesDeclaration.getAttribute(PROPERTY_COMPOSITE_INTERNAL);
        if (internal != null)
        	addProperty(compositeTypeDescription,CST.A_INTERNALINST,internal);
        
        String local = propertiesDeclaration.getAttribute(PROPERTY_COMPOSITE_LOCAL_SCOPE);
        if (local != null)
        	addProperty(compositeTypeDescription,CST.A_LOCALSCOPE,local,"String[]");
 
        String composite = propertiesDeclaration.getAttribute(PROPERTY_COMPOSITE_COMPOSITE_SCOPE);
        if (composite != null)
        	addProperty(compositeTypeDescription,CST.A_COMPOSITESCOPE,composite,"String[]");

        String appli = propertiesDeclaration.getAttribute(PROPERTY_COMPOSITE_APPLI_SCOPE);
        if (appli != null)
        	addProperty(compositeTypeDescription,CST.A_APPLISCOPE,appli,"String[]");
