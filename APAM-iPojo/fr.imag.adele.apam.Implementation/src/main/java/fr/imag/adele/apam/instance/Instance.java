...
public class Instance extends InstanceManager implements ApformInstance {
	...
	public void resolve(Dependency dependency) {
		...
		case IMPLEMENTATION:
			if (dependency.isScalar())
				ApamResolver.newWireImpl(apamInstance, dependency.getTarget(),dependency.getName(), constraints, preferences);
			else
				ApamResolver.newWireImpls(apamInstance, dependency.getTarget(),	dependency.getName(), constraints);
			break;
		case SPECIFICATION:
			if (dependency.isScalar())
				ApamResolver.newWireSpec(apamInstance, null, dependency.getTarget(), dependency.getName(), constraints, preferences);
			else
				ApamResolver.newWireSpecs(apamInstance, null, dependency.getTarget(), dependency.getName(), constraints, preferences);
			break;
		case INTERFACE:
			if (dependency.isScalar())
				ApamResolver.newWireSpec(apamInstance, dependency.getTarget(), null, dependency.getName(), constraints, preferences);
			else
				ApamResolver.newWireSpecs(apamInstance, dependency.getTarget(), null, dependency.getName(), constraints, preferences);
			break;
		}
	}
	...
	public void setState(int state) {
		...
		if (state == VALID)
			getFactory().getApamPlatform().newInstance(getInstanceName(),this);
		
		if (state == INVALID)
			getFactory().getApamPlatform().vanishInstance(getInstanceName());
	} 
