 ...
 * 
 */
public class AdeleTechHeatingApplication implements ChangeListener,
	ActionListener {
    ...
    public void start() {
	System.out
		.println("GenericHeatingApplication : " + myName + ".start()");
    ...
    public void stop() {
	System.out.println("GenericHeatingApplication : " + myName + ".stop()");
	if (mainWindow != null)
	    mainWindow.dispose();
	mainWindow = null;
	...
	mainWindow.setLayout(new BoxLayout(mainWindow.getContentPane(),
		BoxLayout.Y_AXIS));

	JPanel panelSensor = new JPanel();
	panelSensor.setBorder(BorderFactory
		.createTitledBorder("Temperature Sensor"));
	labelTemperature.setPreferredSize(new Dimension(200, 30));
	updateTemperature();
	panelSensor.add(labelTemperature);

	JPanel panelUserSetting = new JPanel();
	panelUserSetting.setBorder(BorderFactory
		.createTitledBorder("User Temperature Setting"));
	panelUserSetting.setLayout(new BoxLayout(panelUserSetting,
		BoxLayout.Y_AXIS));
	...
	panelHeater.setBorder(BorderFactory
		.createTitledBorder("Heater Activity"));
	...
	if (mySensor != null) {
	    currentTemperature = mySensor.getCurrentTemperature();
	    labelTemperature.setText(currentTemperature + "°C");

	} else
	    labelTemperature.setText("No Temperature Sensor");
    }

    void updateHeater() {
	if (myHeater != null) {
	    if (mySensor != null) {
		if (currentTemperature >= userSetting.getValue()) {
		    System.out
			    .println("Current temperature is too high, turning off the Heater");
		    myHeater.turnOff();
		} else {
		    System.out
			    .println("Current temperature is too low, turning on the Heater");
		    myHeater.turnOn();
		}
	    }
		...
		labelHeater.setText("Heater is off");
	} else
	    labelHeater.setText("No Heater");
    }
