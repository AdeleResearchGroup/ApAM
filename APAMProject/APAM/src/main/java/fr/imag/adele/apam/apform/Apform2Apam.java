...
//    static Set<String>  expectedDeployedImpls = new HashSet<String>();
    ...
    /**
     * The event executor. We use a pool of a single single thread to process events in their arrival
     * order and never block the underlying platform thread that is invoking Apform.
     */
    static private final ThreadPoolExecutor executor = new ThreadPoolExecutor(1, 1,
    ...
    private static abstract class ApformEventProcessing implements Runnable {
    	...
    	public void run() {
    		try {
    			context.set(new ProcessingContext());
    			process();
    ...
    public static void waitForImplementation(String implementationName) {
    	...
    	if (getContext() == null) {
            ...
            synchronized (expectedImpls) {
	        	expectedImpls.add(implementationName);
	            try {
	            	while (expectedImpls.contains(implementationName))
	            		expectedImpls.wait();
    	...
    	for (Runnable pendingEvent : executor.getQueue()) {
    	 ...
    	 */
    	executor.getQueue().remove(expectedEvent);
    	expectedEvent.run();
    ...
    private static class InstanceRegistrationProcessing extends ApformEventProcessing {
		...
		public void process() {
	        ...
	        inst = CST.InstBroker.addInst(Apform2Apam.rootInst, instance, instance.getProperties());
		}
    ...
    private static class ImplementationRegistrationProcessing extends ApformEventProcessing {
		...
		public void process() {
	        ...
	        impl = ((ImplementationBrokerImpl) CST.ImplBroker).addImpl(Apform2Apam.rootType, implementation, implementation.getProperties());
	        
	        // wake up any threads waiting for this implementation
	        synchronized (expectedImpls) {
	            if (expectedImpls.contains(implementationName)) { // it is expected
	                expectedImpls.remove(implementationName);
	                expectedImpls.notifyAll(); // wake up the thread waiting in waitForImplementation
	            }
	        }
...
//}
    ...
    public static void newInstance(String instanceName, ApformInstance client) {
    	executor.execute(new InstanceRegistrationProcessing(instanceName, client));
    }
    ...
    public static void newImplementation(String implemName, ApformImplementation client) {
    	executor.execute(new ImplementationRegistrationProcessing(implemName, client));
    }
    ...
    public static void newSpecification(String specName, ApformSpecification client) {
    	executor.execute(new SpecificationRegistrationProcessing(specName, client));
   }
