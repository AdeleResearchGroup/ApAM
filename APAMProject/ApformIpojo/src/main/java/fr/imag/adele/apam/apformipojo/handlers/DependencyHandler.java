...
public class DependencyHandler extends ImplementationHandler {
    ...
    public void initializeComponentFactory(ComponentTypeDescription componentDescriptor, Element componentMetadata)
        ...
        ApformIpojoImplementation.Description implementationDescription = isApamImplementation ? (ApformIpojoImplementation.Description) componentDescriptor
                : null;
        ...
        try {
            ...
            throw new ConfigurationException("iPojo component "
                    + ImplementationHandler.quote(componentDescriptor.getName()) + ": "
        ...
        Element dependencyDeclarations[] = componentMetadata.getElements(
                ApformIpojoImplementation.DEPENDENCY_DECLARATION, ImplementationHandler.APAM_NAMESPACE);
        for (Element dependencyDeclaration : dependencyDeclarations) {

            String dependencyName = dependencyDeclaration.getAttribute(DependencyHandler.DEPENDENCY_NAME_PROPERTY);
            String dependencyFieldNames = dependencyDeclaration
                    .getAttribute(DependencyHandler.DEPENDENCY_FIELD_PROPERTY);
            String dependencySourceNames = dependencyDeclaration
                    .getAttribute(DependencyHandler.DEPENDENCY_SOURCE_PROPERTY);
            ...
            if (dependencyFieldNames == null) {
                String alternateDependencyFieldNames = dependencyDeclaration
                        .getAttribute(DependencyHandler.DEPENDENCY_FIELD_ALTERNATE_PROPERTY);
                if (alternateDependencyFieldNames != null) {
                    dependencyFieldNames = alternateDependencyFieldNames;
                    dependencyDeclaration.addAttribute(new Attribute(DependencyHandler.DEPENDENCY_FIELD_PROPERTY,
            ...
            if (hasInstrumentedCode && (dependencyFieldNames == null)) {
                throw new ConfigurationException("APAM Dependency " + ImplementationHandler.quote(implementationName)
                        + ": "
            ...
            if (dependencyName == null) {
                ...
                dependencyDeclaration.addAttribute(new Attribute(DependencyHandler.DEPENDENCY_NAME_PROPERTY,
                        dependencyName));
            }
            ...
            if ((dependencySpecification != null) && (dependencyImplementation != null)) {
                throw new ConfigurationException("APAM Dependency "
                        + ImplementationHandler.quote(implementationName + "." + dependencyName) + ": "
            ...
            if (hasInstrumentedCode) {
                for (String fieldName : ParseUtils.parseArrays(dependencyFieldNames)) {
                    ...
                    try {
                        ...
                        boolean isCollectionField = fieldClass.isArray()
                                || Collection.class.isAssignableFrom(fieldClass);

                        if (getFactory().getPojoMetadata().getField(fieldName) == null) {
                            throw new ConfigurationException("APAM Dependency "
                                    + ImplementationHandler.quote(implementationName + "." + dependencyName) + ": "
                                            + "the specified field " + ImplementationHandler.quote(fieldName)
                                    + " is not instrumented by iPojo");
                        }
                        ...
                        if (isCollectionField && !Dependency.isSupportedCollection(field)) {
                            throw new ConfigurationException("APAM Dependency "
                                    + ImplementationHandler.quote(implementationName + "." + dependencyName) + ": "
                                    + "the specified field " + ImplementationHandler.quote(fieldName)
                                    + " must be of type " + Dependency.supportedCollectionClasses());
                        }
                        ...
                        if (!isDependencyAggregateSet) {
                            ...
                            dependencyDeclaration.addAttribute(new Attribute(
                                    DependencyHandler.DEPENDENCY_AGGREGATE_PROPERTY, dependencyAggregate));
                        }
                        ...
                        if (isCollectionField && !isDependencyAggregate) {
                            throw new ConfigurationException("APAM Dependency "
                                    + ImplementationHandler.quote(implementationName + "." + dependencyName) + ": "
                                            + "the class of the field " + ImplementationHandler.quote(fieldName)
                                    + " cannot be a Collection or array");
                        }
                        ...
                        Class<?> fieldElementClass = isDependencyAggregate ? Dependency.getCollectionElement(field)
                                : field.getType();
                            ...
                            try {
                                ...
                                if (!fieldElementClass.isAssignableFrom(interfaceClass)) {
                                    throw new ConfigurationException("APAM Dependency "
                                            ...
                                            + ImplementationHandler.quote(dependencyInterface) + " "
                                                    + "can not be assigned to field "
                                            + ImplementationHandler.quote(fieldName));
                                }
                                ...
                                throw new ConfigurationException("APAM Dependency "
                                        + ImplementationHandler.quote(implementationName + "." + dependencyName) + ": "
                                                    + "the specified interface "
                                        + ImplementationHandler.quote(dependencyInterface) + " is not accessible");
                            }
                        }

                        if ((fieldElementClass != null) && (dependencySpecification != null)
                                && !fieldElementClass.isInterface()) {
                            throw new ConfigurationException("APAM Dependency "
                                    + ImplementationHandler.quote(implementationName + "." + dependencyName) + ": "
                                    + "the specified field " + ImplementationHandler.quote(fieldName) + " "
                                    + " must be declared using an interface provided by specification "
                                    + ImplementationHandler.quote(dependencySpecification));
                        }
                        ...
                        if ((dependencySpecification == null) && (dependencyImplementation == null)
                            ...
                            if (fieldElementClass != null) {
                                ...
                                if (dependencyInterface != null)
                                    dependencyDeclaration.addAttribute(new Attribute(
                                            DependencyHandler.DEPENDENCY_INTERFACE_PROPERTY, dependencyInterface));
                            }
                        }
                        ...
                        throw new ConfigurationException("APAM Dependency "
                                + ImplementationHandler.quote(implementationName + "." + dependencyName) + ": "
                                + "the specified field " + ImplementationHandler.quote(fieldName)
                                + " is not declared in the implementation class");
                    }
            ...
            Element constraintsDeclarations[] = dependencyDeclaration.getElements(
                    DependencyHandler.CONSTRAINTS_DECLARATION, ImplementationHandler.APAM_NAMESPACE);

            for (Element constraintsDeclaration : ImplementationHandler.optional(constraintsDeclarations)) {

                Element constraintDeclarations[] = constraintsDeclaration.getElements(
                        DependencyHandler.CONSTRAINT_DECLARATION, ImplementationHandler.APAM_NAMESPACE);
                for (Element constraintDeclaration : ImplementationHandler.optional(constraintDeclarations)) {

                    String filter = constraintDeclaration.getAttribute(DependencyHandler.CONSTRAINT_FILTER_PROPERTY);
                    if (filter == null)
                        throw new ConfigurationException("APAM Dependency "
                                + ImplementationHandler.quote(implementationName + "." + dependencyName) + ": "
            ...
            Element preferencesDeclarations[] = dependencyDeclaration.getElements(
                    DependencyHandler.PREFERENCES_DECLARATION, ImplementationHandler.APAM_NAMESPACE);

            for (Element preferencesDeclaration : ImplementationHandler.optional(preferencesDeclarations)) {

                Element constraintDeclarations[] = preferencesDeclaration.getElements(
                        DependencyHandler.CONSTRAINT_DECLARATION, ImplementationHandler.APAM_NAMESPACE);
                for (Element constraintDeclaration : ImplementationHandler.optional(constraintDeclarations)) {
            ...
            dependency.source = (hasInstrumentedCode || isAbstract) ? new String[0] : ParseUtils
                    .parseArrays(dependencySourceNames);
    ...
    public void configure(Element componentMetadata, Dictionary configuration)
        ...
        Element dependencyDeclarations[] = componentMetadata.getElements(
                ApformIpojoImplementation.DEPENDENCY_DECLARATION,
        ...
        for (Element dependencyDeclaration : dependencyDeclarations) {
            ...
            for (Element constraintsDeclaration : ImplementationHandler.optional(constraintsDeclarations)) {
                ...
                for (Element constraintDeclaration : ImplementationHandler.optional(constraintDeclarations)) {
                    try {
                        constraints.add(getFactory().getBundleContext().createFilter(
                                constraintDeclaration.getAttribute(DependencyHandler.CONSTRAINT_FILTER_PROPERTY)));
            ...
            for (Element preferencesDeclaration : ImplementationHandler.optional(preferencesDeclarations)) {
                ...
                for (Element constraintDeclaration : ImplementationHandler.optional(constraintDeclarations)) {
                    try {
                        preferences.add(getFactory().getBundleContext().createFilter(
                                constraintDeclaration.getAttribute(DependencyHandler.CONSTRAINT_FILTER_PROPERTY)));
                    } catch (InvalidSyntaxException ignored) {
                    }
                }
