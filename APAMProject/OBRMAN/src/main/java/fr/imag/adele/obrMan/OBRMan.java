...
//        System.out.println("OBR >>> " +obr );
    ...
    private Implementation resolveSpec(CompositeType compoType, ResolvableReference resource,
        ...
        if (resource instanceof SpecificationReference) {
            selected = obr.lookFor(compoType, OBR.CAPABILITY_COMPONENT, "(provide-specification="
                    + resource.as(SpecificationReference.class).getName() + ")",
                    constraints, preferences);
        }
        if (resource instanceof InterfaceReference) {
            selected = obr.lookFor(compoType,OBR.CAPABILITY_COMPONENT, "(provide-interfaces=*;" + resource.as(InterfaceReference.class).getJavaType() + ";*)",
                    constraints, preferences);
        }
        if (resource instanceof MessageReference) {
            selected = obr.lookFor(compoType,OBR.CAPABILITY_COMPONENT, "(provide-messages=*;" + resource.as(MessageReference.class).getJavaType() + ";*)",
                    constraints, preferences);
        }
        if (selected != null) {
    ...
    public Implementation findImplByName(CompositeType compoType, String implName) {
		...
		Selected selected = obr.lookFor(compoType,OBR.CAPABILITY_COMPONENT, "(name=" + implName + ")", null, null);
    ...
    public Specification findSpecByName(CompositeType compoType, String specName) {
		...
		Selected selected = OBRMan.obr.lookFor(compoType,OBR.CAPABILITY_COMPONENT, "(name=" + specName + ")", null, null);
