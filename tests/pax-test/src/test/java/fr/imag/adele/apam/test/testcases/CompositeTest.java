...
public class CompositeTest extends ExtensionAbstract {
	...
	public void ComponentMngtMainCompositeAccessItsPrivateImpls_tc118() {
		...
		Implementation dependencyOfA=CST.apamResolver.findImplByName(null,"BoschSwitch");
		CompositeType rootComposite=(CompositeType) CST.apamResolver.findImplByName(null,CST.ROOT_COMPOSITE_TYPE);
		...
		Implementation groupAImpl=CST.apamResolver.findImplByName(null,"group-a");
		
		Instance groupAInstance=groupAImpl.createInstance(composite_a, Collections.<String,String>emptyMap());
		...
		String message=String.format(messageTemplate, "A should have visibility to the implementation, just NOT export them, so the composite A should be able to create an instance of it");
